...
if(!$dbconfig['user']||!$dbconfig['pass']||!$dbconfig['name']) {
	...
	$date = date("Y-m-d");
	$DB->query("INSERT INTO `ytidc_fenzhan`(`domain`, `title`, `subtitle`, `description`, `keywords`, `notice`, `admin`, `password`, `user`, `status`) VALUES ('{$_SERVER['HTTP_HOST']}','默认站点','使用云塔IDC系统','这个是默认站点，删除后可能无法浏览后台，可修改信息但请勿删除！','','这个是默认站点，删除后可能无法浏览后台，可修改信息但请勿删除！','admin','123456','0','1')");
}
if($e==0) {
